...
const Recipe = ({ recipe }) => {
    ...
    const [showSecondSVGIngredients, setShowSecondSVGIngredients] = useState(Array(recipe.ingredients.length).fill(false));
    const [showSecondSVGInstructions, setShowSecondSVGInstructions] = useState(Array(recipe.instructions.length).fill(false));
  
    const toggleSVG = (index, section) => {
      const updatedSVGState = section === 'ingredients' ? [...showSecondSVGIngredients] : [...showSecondSVGInstructions];
    ...
    <div className={styles.recipeInfo}>
      ...
      <ul>
        {recipe.ingredients.map((ingredient, index) => (
          <div key={index} className={styles["ingredient-box"]}>
            ...
            <div className={styles["transparent-box"]}></div>
            <div onClick={() => toggleSVG(index)}>
...
<span className={styles.serving}> /person </span>
        </div>
      </div>
      ...
      <div className={styles.rightColumn}>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="#ffa101" d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10s10-4.477 10-10S17.523 2 12 2zm0 18l2-2l1-1v-2h-2v-1l-1-1H9v3l2 2v1.931C7.06 19.436 4 16.072 4 12l1 1h2v-2h2l3-3V6h-2L9 5v-.411a7.945 7.945 0 0 1 6 0V6l-1 1v2l1 1l3.13-3.13A7.983 7.983 0 0 1 19.736 10H18l-2 2v2l1 1h2l.286.286C18.029 18.061 15.239 20 12 20z"/></svg></span>
        <span className={styles.timee}>
          <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
            ...
            <path fill="#ffa101" d="M17.188 8.364a.75.75 0 0 0-1.052-1.052l-3.17 2.465l-2.072 1.48a1.684 1.684 0 1 0 2.35 2.349l1.479-2.072l2.465-3.17Z"/>
          </svg>
          {recipe.prep_time.time} {recipe.prep_time.unit}
        </span>
        {/* Likes */}
        <span className={styles.likes}>
        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="#fa1" d="m12.1 18.55l-.1.1l-.11-.1C7.14 14.24 4 11.39 4 8.5C4 6.5 5.5 5 7.5 5c1.54 0 3.04 1 3.57 2.36h1.86C13.46 6 14.96 5 16.5 5c2 0 3.5 1.5 3.5 3.5c0 2.89-3.14 5.74-7.9 10.05M16.5 3c-1.74 0-3.41.81-4.5 2.08C10.91 3.81 9.24 3 7.5 3C4.42 3 2 5.41 2 8.5c0 3.77 3.4 6.86 8.55 11.53L12 21.35l1.45-1.32C18.6 15.36 22 12.27 22 8.5C22 5.41 19.58 3 16.5 3Z"/></svg>
          {recipe.likes}
        </span>
...
{recipe.chef_note && (
  <div>
    ...
    <p className={styles.NoteContent}>{recipe.chef_note}</p>
  </div>
)}
